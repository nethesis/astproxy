...
var PJSIP_EXTEN_INIT_RETRY_DELAY = 2000;
var pjsipExtenInitRetryTimer;

function requestPjsipExtenInitialization() {
  astProxy.doCmd({
...
function start() {
  try {
    ...
    requestPjsipExtenInitialization();

    if (!reloading) {
      emitter.once('pjsipExtenInitialized', () => {
        // initialize queues
        astProxy.doCmd({
          command: 'listQueues'
        }, initializeQueues);
...
function initializePjsipExten(err, results) {
  try {
    if (err) {
      ...
      if (!pjsipExtenInitRetryTimer) {
        pjsipExtenInitRetryTimer = setTimeout(() => {
          pjsipExtenInitRetryTimer = undefined;
          requestPjsipExtenInitialization();
        }, PJSIP_EXTEN_INIT_RETRY_DELAY);
      }
      return;
    }
    if (pjsipExtenInitRetryTimer) {
      clearTimeout(pjsipExtenInitRetryTimer);
      pjsipExtenInitRetryTimer = undefined;
    }
