 ...
 */
var recordingConv = {};
var recordingConvChannels = {};

function getRecordingConvKey(extension, convid) {
  return extension + '|' + convid;
}

function getRecordingConvStatus(extension, convid) {
  return recordingConv[getRecordingConvKey(extension, convid)] || recordingConv[convid];
}

function isRecordingConversation(extension, convid) {
  return getRecordingConvStatus(extension, convid) !== undefined;
}

function stopRecordConversationForExtension(extension, convid, cb) {
  try {
    var recordingKey = getRecordingConvKey(extension, convid);
    var chid = recordingConvChannels[recordingKey] || getExtenIdChannelConversation(extension, convid);
    if (!chid) {
      logger.warn(IDLOG, 'no channel to stop record of conversation ' + convid + ' of exten ' + extension);
      cb();
      return;
    }
    ...
    astProxy.doCmd({
      ...
      cb(err);
      stopRecordCallCb(err, convid, extension);
    });
  } catch (err) {
...
function addConversationToExten(exten, resp, chid, spyConnected) {
  try {
    ...
    if ((extensions[exten] || tempExtensions[exten]) && resp[chid].bridgedChannel !== undefined) {
      ...
      var recordingStatus = getRecordingConvStatus(exten, convid);
      if (recordingStatus !== undefined) {
        logger.info(IDLOG, 'set recording status to conversation ' + convid);

        if (recordingStatus === RECORDING_STATUS.TRUE) {
...
function addConversationToTrunk(trunk, resp, chid) {
  try {
    ...
    if (isTrunk(trunk)) {
      ...
      // if the conversation is recording, sets its recording status
      var recordingStatus = recordingConv[convid];
      if (recordingStatus !== undefined) {
        logger.info(IDLOG, 'set recording status to conversation ' + convid);
...
function evtHangupConversation(data) {
  try {
    ...
    Object.keys(recordingConv).forEach(function(recordingKey) {
      var keyPrefix = data.channelExten + '|';
      if (recordingKey.indexOf(keyPrefix) === 0) {
        var recordedConvid = recordingKey.substring(keyPrefix.length);
        stopRecordConversationForExtension(data.channelExten, recordedConvid, function(err) {
          if (err) {
            logger.error(IDLOG, 'stopping record on hangup of exten ' + data.channelExten + ' failed: ' + err.toString());
          }
        });
      } else if (recordingKey.indexOf(data.channel) !== -1) {
        delete recordingConv[recordingKey];
      }
    });
  ...
  try {
    ...
    if (extensions[extension]) {
      ...
      if (chToTransfer !== undefined) {
        var executeBlindTransfer = function() {
        ...
        if (isRecordingConversation(extension, convid)) {
          stopRecordConversationBeforeTransfer(extension, convid, function(err) {
            if (err) {
              logger.warn(IDLOG, 'continue blind transfer after recording stop failure: ' + err.toString());
            }
...
function attendedTransferConversation(extension, convid, to, cb) {
  try {
    ...
    if (extensions[extension]) {
      ...
      if (chToTransfer !== undefined) {
        var executeAttendedTransfer = function() {
  ...
  try {
    ...
    if (extensions[extension]) {
      ...
      if (!isRecordingConversation(extension, convid)) {
        ...
        stopRecordConversationForExtension(extension, convid, cb);
      }
...
function stopRecordConversationBeforeTransfer(extension, convid, cb) {
  try {
    if (!isRecordingConversation(extension, convid)) {
      cb();
      return;
    ...
    var chid = getExtenIdChannelConversation(extension, convid);
    if (!chid) {
      logger.warn(IDLOG, 'no channel to stop record before transfer of conversation ' + convid + ' of exten ' + extension);
      cb();
      return;
    }

    logger.info(IDLOG, 'stop recording before transfer of conversation ' + convid + ' of exten ' + extension);
    astProxy.doCmd({
      command: 'stopRecordCall',
      channel: chid
    }, function (err) {
      if (err) {
        logger.error(IDLOG, 'stop recording before transfer of conversation ' + convid + ' failed: ' + err.toString());
      }
      stopRecordCallCb(err, convid, extension);
      cb(err);
    });
      ...
      if (!isRecordingConversation(extension, convid)) {
        ...
        astProxy.doCmd({
          ...
          try {
            ...
            setRecordStatusMuteConversations(convid, extension);
            cb();
      ...
      if (!isRecordingConversation(extension, convid)) {
        ...
        astProxy.doCmd({
          ...
          try {
            ...
            recordingConv[getRecordingConvKey(extension, convid)] = RECORDING_STATUS.TRUE;
            // set the recording status of all conversations with specified convid
            setRecordStatusConversations(convid, true, extension);
            cb();
...
function startRecordConversation(extension, convid, cb) {
  try {
    ...
    if (extensions[extension]) {
      ...
      // compute cdrChid: the "local" (non-PJSIP) channel partner in this bridge.
      // Using this channel for CDR(recordingfile) avoids contaminating the CDR rows
      // of other call legs that share the same PJSIP channel through bridge-sync.
      //   - initial call (inbound queue):  source = Local/201@queue;2   → only updates that CDR row
      //   - consultation (201→202):        dest   = Local/202@from-internal;1 → only updates that row
      //   - 202 receiving:                 source = Local/202@from-internal;2 → only updates post-transfer row
      var cdrChid = chid;
      var cdrConv = extensions[extension].getConversation(convid);
      if (cdrConv) {
        var cdrSource = cdrConv.getSourceChannel();
        var cdrDest = cdrConv.getDestinationChannel();
        if (cdrSource && !cdrSource.isExtension(extension)) {
          cdrChid = cdrSource.getChannel();
        } else if (cdrDest && !cdrDest.isExtension(extension)) {
          cdrChid = cdrDest.getChannel();
        }
      }
      ...
      if (isRecordingConversation(extension, convid)) {
        ...
        async.series([
          ...
          function (callback) {
            ...
            logger.info(IDLOG, 'set "CDR(recordingfile)" asterisk variable on ' + cdrChid);
            astProxy.doCmd({
              command: 'setVariable',
              name: 'CDR(recordingfile)',
              value: filename,
              channel: cdrChid
            }, function (err) {
              try {
          ...
          astProxy.doCmd({
            ...
            try {
              ...
              recordingConvChannels[getRecordingConvKey(extension, convid)] = chid;
              // set the recording status of the conversation
              startRecordCallCb(convid, extension);
              cb();
 ...
 * @private
 */
function stopRecordCallCb(err, convid, extension) {
  try {
    if (err) {
      ...
      if (extension) {
        delete recordingConv[getRecordingConvKey(extension, convid)];
        delete recordingConvChannels[getRecordingConvKey(extension, convid)];
      } else {
        delete recordingConv[convid];
      }
      // reset the recording status of all conversations with specified convid
      setRecordStatusConversations(convid, false, extension);
    }
  } catch (error) {
    logger.error(IDLOG, error.stack);
  }
 ...
 * @private
 */
function startRecordCallCb(convid, extension) {
  try {
 ...
 * @private
 */
function setRecordStatusConversations(convid, value, extension) {
  try {
    // check parameters
    ...
    for (exten in extensions) { // cycle in all extensions
      ...
      if (convs) {
        ...
        for (cid in convs) {
          ...
          if (cid === convid && (!extension || exten === extension)) {
 ...
 * @private
 */
function setRecordStatusMuteConversations(convid, extension) {
  try {
    // check parameter
    if (typeof convid !== 'string') {
      throw new Error('wrong parameters: ' + JSON.stringify(arguments));
    ...
    for (exten in extensions) { // cycle in all extensions
      ...
      if (convs) {
        ...
        for (cid in convs) {
          ...
          if (cid === convid && (!extension || exten === extension)) {
            if (extension) {
              recordingConv[getRecordingConvKey(extension, convid)] = RECORDING_STATUS.MUTE;
            } else {
