      ...
      data: function(data) {
        try {
          if (data.event === 'Parkinglot' && data.startspace && data.stopspace && data.timeout) {
            ...
            if (map[actionid]) {
              map[actionid](new Error(data.message));
              delete map[actionid];
            }
            list = {};
          } else if (data.response === 'Error') {
            var actionidFallback = data.actionid;
            if (map[actionidFallback]) {
              map[actionidFallback](new Error('error'));
              delete map[actionidFallback];
            }
            list = {};
          }
